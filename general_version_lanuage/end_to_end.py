...
def parse_args():
    ...
    parser.add_argument("--sup_num", type=float, default=22, help="supervised sample number in one epoch")
...
def train(args,image_array,caption_array,mode="train"):
    ...
    for epoch_idx in range(args.epochs):
        ...
        for batch in tqdm(dl):
            ...
            loss=0.15*loss_image+0.15*loss_text+0.7*loss_cluster+0.2*loss_mmd+0*loss_average+1.0*loss_clip#+0.5*loss_kde+0.0005*loss_text+loss_kde1
        ...
        if(epoch_idx==29):
            # 模型保存
            torch.save(model, 'model_end2end_'+str(epoch_idx)+'_15000_5000_improve_new_relative_distance_kl.pkl')
